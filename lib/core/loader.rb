    ...
    module Singleton
      ...
      def set_current_library(library)
        if library
          ...
          @current_library = library.is_a?(Symbol) ? Loader.libraries[library] : library
        else
          base_class = nil
          @current_library = Loader.default_library || (base_class ? Loader.create_library(base_class) :  {})
        end
      end
